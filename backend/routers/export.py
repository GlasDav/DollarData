...
def export_report_pdf(
    ...
    try:
        print(f"DEBUG: Starting PDF export for {start_date} to {end_date}")
    ...
    print(f"DEBUG: PDF Export - Income: {len(inc_values)}, Expenses: {len(exp_values)}")
    ...
    def create_donut_drawing(values, colors_list, width=125, height=125):
        ...
        pie.innerRadius = (width - 20) * 0.35 # Donut Hole
        pie.data = values
        pie.labels = None
